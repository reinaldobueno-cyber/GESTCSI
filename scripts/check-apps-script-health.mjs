...
export const EXPECTED_APPS_SCRIPT_VERSION = '2026-09-15-inventory-paging-v1';
...
export async function checkHealthOnce(options = {}) {
  ...
  try {
    ...
    if (options.authToken && (payload.visibility !== 'authenticated' || !payload.dependencies)) {
      throw new Error('authenticated_detail_missing');
    }
...
export async function monitorHealth(options = {}) {
  ...
  if (options.alert) await options.alert(incident);
  return incident;
}
...
async function runCli() {
  const result = await monitorHealth({
    ...
    authToken: process.env.GESTCSI_HEALTH_AUTH_TOKEN || '',
