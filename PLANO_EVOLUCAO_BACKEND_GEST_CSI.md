...
- **Progresso em 15/09/2026:** [`MATRIZ_AUTORIZACAO_LEGADO_E05.md`](./MATRIZ_AUTORIZACAO_LEGADO_E05.md) registra as 56 ações, aliases, papéis e pendências de transporte. Guarda HTTP default-deny e teste de quatro perfis implementados na branch. Os 36 comandos classificados como POST obrigatório recusam GET; os 3 refreshes condicionais também foram deslocados para POST. Sessão e status de adoção deixaram de gravar por GET; as 20 leituras, inclusive os 2 diagnósticos finais, foram verificadas sem escrita externa no código da branch. Inventário histórico ausente/vazio agora sinaliza fonte indisponível em vez de sucesso com zero projetos. Webhook falha fechado sem token, mas não foi publicado. O guia oficial do Google restringe JSONP a dados não sensíveis; o corte exige transporte autenticado compatível com a origem do painel. [`DECISAO_TRANSPORTE_E05_E09.md`](./DECISAO_TRANSPORTE_E05_E09.md) explicita que a ponte de mesma origem depende de servir painel e API no mesmo domínio, além de sessão, CSRF, auditoria e smoke antes do corte. Ainda faltam adaptação do painel, idempotência/auditoria e smoke real, portanto E05 não está OK.
- **Correção de apresentação em 15/09/2026 (branch):** timeout ou resposta histórica abaixo de 201 projetos agora marca o inventário como indisponível; sem cache íntegro, histórico mostra “—” e total ClickUp fica parcial “≥ projetos 2026”, sem esconder a carteira atual. Não é restauração dos projetos `<=2025` em produção.
- **Ponte de homologação em 15/09/2026 (branch):** [`gateway/`](./gateway/) prepara painel e API sob a mesma origem com login em POST, sessão opaca, logout com CSRF e inventário histórico paginado. Fica desligada por padrão, sem domínio, D1, secrets ou implantação; mutações permanecem fechadas e o painel ainda usa JSONP. E05 segue pendente de testes em runtime, adaptação do cliente e smoke real dos 337 históricos/215 atuais.
